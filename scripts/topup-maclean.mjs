...
const queries = [
  'Maclean-Fogg news',
  'Maclean Power Systems news',
  'maclean-fogg.com news',
  'Maclean-Fogg Centerbridge',
  'Maclean-Fogg family 100 years',
  'Maclean-Fogg Sustainability report 2025',
  'MPS MacLean Power news',
  'Maclean-Fogg Pressed Metals news',
  'Maclean-Fogg steel fastener news',
];
for (const q of queries) {
  if (candidates.length >= need * 5) break;
  try {
    const r = await fetchBingNews(q, { maxResults: 10 });
    for (const x of r) {
...
console.log(`  filtered (title contains 'maclean'): ${filtered.length}`);
for (const x of filtered) seen.add(x.url);
filtered.slice(0,5).forEach(x => console.log('    -', x.title.slice(0,60), '|', x.url.slice(0,60)));
...
await Promise.all(Array.from({ length: 8 }, async () => {
  while (queue.length && validated.length < need) {
    ...
    if (date && date < CUTOFF) { console.log(`  skip old: ${cand.title.slice(0,50)} (${date.toISOString().slice(0,10)})`); continue; }
